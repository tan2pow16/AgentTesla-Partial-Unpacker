 ...
 *
 * For alpha-enabled samples with a 150x150 random junk square at
...
function decode(_fpath, discard_dim, key)
{
  ...
  let nx = discard_dim || 0;
  let ny = discard_dim || 0;
  ...
  let cache = 0;
  let swap = [2, 1, 0, 3];
  for(let i = 0 ; i < img.height - ny ; i++)
  {
    for(let j = 0 ; j < img.width - nx ; j++)
    {
...
function __main__(args)
{
  ...
  if(args.length < 1)
  {
    console.log('node %s <path/to/encoded.png> [discarded-pixel-width] [hex-key]', path.basename(__filename));
    process.exitCode = 1;
    return;
  }
